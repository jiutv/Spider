...
public class ZhaiNanYS extends Spider {
    ...
    public String homeVideoContent() throws Exception {
        ...
        for (String[] tab : HOME_TABS) {
            String tid = tab[0];
            String url = API + "?ac=list&t=" + tid + "&page=1&limit=" + HOME_TAB_SIZE;
    ...
    public String categoryContent(String tid, String pg, boolean filter, HashMap<String, String> extend) throws Exception {
        ...
        String url = API + "?ac=list&t=" + useTid + "&page=" + page + "&limit=" + PAGE_SIZE;
        JSONObject root = fetchJson(url);
    ...
    public String searchContent(String key, boolean quick, String pg) throws Exception {
        int page = parsePage(pg);
        String url = API + "?ac=list&wd=" + URLEncoder.encode(key, "UTF-8") + "&page=" + page + "&limit=" + PAGE_SIZE;
        JSONObject root = fetchJson(url);
        List<String> ids = new ArrayList<>();
        List<Vod> vods = parseVodList(root, ids);
        int total = root.optInt("total", 0);
    ...
    /** 批量 detail 补图 (PHP vod 支持 ac=detail&ids=1,2,3 批量查询) */
